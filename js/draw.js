...
var draw_graph = function(graphType,obj){
	...
	var mousedown = function(e){
		...
		if(graphType == 'pencil'){
			...
			var ghSize = 1;
			context.clearRect(startX - ghSize * 10 ,  startY - ghSize * 10 , ghSize * 20 , ghSize * 20);				
		}	
	};	
	...
	var  mousemove = function(e){
		...
		//涂鸦 未画得时候 出现一个小圆
			...
			if(canDraw){
				...
				context_bak.arc(x,y,hwSize*10,0,Math.PI * 2,false);		
				context_bak.fill();
				context_bak.stroke();
				context_bak.restore();
			}else{	
		...
		//橡皮擦 不管有没有在画都出现小方块 按下鼠标 开始清空区域
			...
			context_bak.moveTo(x - ghSize * 10 ,  y - ghSize * 10 );						
			context_bak.lineTo(x + ghSize * 10  , y - ghSize * 10 );
			context_bak.lineTo(x + ghSize * 10  , y + ghSize * 10 );
			context_bak.lineTo(x - ghSize * 10  , y + ghSize * 10 );
			context_bak.lineTo(x - ghSize * 10  , y - ghSize * 10 );	
			context_bak.stroke();		
			if(canDraw){							
				context.clearRect(x - ghSize * 10 ,  y - ghSize * 10 , ghSize * 20 , ghSize * 20);
