...
const export2Excel = (theadData, tbodyData, dataname) => {
    ...
    let width = 300; // 设置图片大小
    let height = 200;

    // 添加表头信息
            let thead = `<thead><div style="font-size: 17px;font-weight: bold"><span>任务创建日期:${taskCreateDate}</span> &nbsp&nbsp&nbsp<span>任务创建人：${createUserName}</span>&nbsp&nbsp&nbsp<span>任务名称：${taskName}</span>&nbsp&nbsp&nbsp<span>文件夹路径：${folderName}</span>&nbsp&nbsp&nbsp<span>任务状态：${status}</span></div><tr>`;
